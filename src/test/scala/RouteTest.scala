...
class RouteTest extends AnyWordSpec with Matchers with ScalatestRouteTest with MockFactory{
  ...
  "The service" should {
    ...
    "successful response" in {
      ...
      (Service.processData _).expects(sum -> data).returning(Option(expected)).once()
    ...
    "unsuccessful response" in {
      ...
      lazy val sum = 7
      (Service.processData _).expects(sum -> data).throws(new NumberFormatException("Error message")).once()
      Post(s"/upload/$sum/", data) ~> Routes ~> check {
